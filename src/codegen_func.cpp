...
void CodeGenerator::emitFunc(const FuncSymbol& fs) {
    ...
    if (fs.decl) {
        ...
        for (size_t i = 0; i < fs.decl->params.size(); i++) {
            ...
            if (fs.decl->params[i].type.kind == Type::Kind::Float) {
                ...
                if (ireg < 6) {
                    *out_ << "\tmovq\t%" << regs[ireg++] << ", " << loc << "\n";
                    if (isRefCounted(fs.decl->params[i].type)) {
                        rcVars_.back().push_back(fs.decl->params[i].name);
                        *out_ << "\tpushq\t%rax\n";
                        emitRCRetain(regs[ireg-1]); // Retain parameter
                        *out_ << "\tpopq\t%rax\n";
                    }
                }
            }
    ...
    *out_ << currentEndLabel_ << ":\n";
    ...
    // Function-level cleanup (parameters)
    *out_ << "\tpushq\t%rax\n";
    for (auto it = rcVars_.back().rbegin(); it != rcVars_.back().rend(); ++it) {
        emitRCRelease(*it);
    }
    *out_ << "\tpopq\t%rax\n";
    for (auto it = deferStack_.back().rbegin(); it != deferStack_.back().rend(); ++it) {
        emitStmt(*it);
    }
    rcVars_.pop_back();
